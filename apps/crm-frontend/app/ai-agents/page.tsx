...
export default function AiAgentsPage() {
    ...
    const [testMode, setTestMode] = useState<{ active: boolean, agentId: string | null, history: any[] }>({ active: false, agentId: null, history: [] });
    ...
    return (
        <div className="flex h-screen bg-gray-50">
            <Sidebar
                ...
                onChangeView={(view) => router.push(`/${view === 'dashboard' ? '' : view}`)}
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div className={`px-3 py-1 rounded-full text-xs font-semibold 
                                            ${agent.provider === 'OPENAI' ? 'bg-green-100 text-green-800' :
                                                agent.provider === 'GEMINI' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
                                            {agent.provider}
                                        </div>
                                        <div className="flex space-x-2">
                                            ...
                                            <button
                                                onClick={() => {
                                                    setTestMode({ active: true, agentId: agent.id, history: [] });
                                                }}
            ...
            {testMode.active && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-xl max-w-lg w-full flex flex-col h-[600px]">
                        <div className="p-4 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
                            <h3 className="font-bold">Probando Agente</h3>
                            <button onClick={() => setTestMode({ active: false, agentId: null, history: [] })} className="text-gray-500 hover:text-gray-700">✕</button>
                        </div>

                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            {testMode.history.length === 0 && (
                                <div className="text-center text-gray-400 text-sm mt-10">Escribe un mensaje para probar el agente...</div>
                            )}
                            {testMode.history.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[80%] rounded-lg p-3 text-sm ${msg.role === 'user'
                                        ? 'bg-indigo-600 text-white'
                                        : msg.role === 'system' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                                        }`}>
                                        {msg.content}
                                    </div>
                                </div>
                            ...
                            {testLoading && (
                                <div className="flex justify-start">
                                    <div className="bg-gray-100 rounded-lg p-3 text-sm animate-pulse">Escribiendo...</div>
                                </div>
                            )}
                        </div>
