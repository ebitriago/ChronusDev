...
// Requires either:
// 1. App Password (if 2FA enabled): https://myaccount.google.com/apppasswords
// 2. Less secure apps (if no 2FA): Not recommended
...
export async function verifyEmailConnection(): Promise<boolean> {
    if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
        console.log('[Email] Gmail credentials not configured. Set GMAIL_USER and GMAIL_APP_PASSWORD in .env');
        return false;
    }
    ...
    try {
        ...
        console.error('[Email] ❌ Gmail SMTP connection failed:', err);
        return false;
    }
}
...
export async function sendEmail(options: EmailOptions): Promise<{ success: boolean; messageId?: string; error?: string }> {
    ...
    try {
        const info = await transporter.sendMail({
            from: `"ChronusCRM" <${process.env.GMAIL_USER}>`,
            to: options.to,
